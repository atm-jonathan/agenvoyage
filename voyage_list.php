...
$permissiontoadd = $user->rights->voyage->voyage->write;
$permissiontodelete = $user->rights->voyage->voyage->delete;

// Security check
if (empty($conf->voyage->enabled)) {
...
if (empty($reshook)) {
	...
	if ($action == "confirm-add-propal" && !empty($user->rights->propal->creer)) {
					...
					foreach ($toselect as $voyageId) {
						...
						if ($res > 0) {
							...
							$resAddline = $propal->addline(
								...
								'',
								'',
								array('options_fk_voyage' => $voyage->id)
							);
							if ($resAddline > 0) {
								$numberLineCreate++;
							} else {
								$numberLineError++;
								setEventMessage($langs->trans('CHIErrorAddVoyageLine') . ' : ' . $propal->errorsToString(), 'errors');
								$error++;
							}
							$voyage->add_object_linked('propal',$propal->id);
						} else {
							setEventMessage($langs->trans('CHIErrorFetchVoyage') . ' : ' . $voyage->errorsToString(), 'errors');
							$error++;
						}
					}
				} else {
		...
		if ($error == 0) {
			setEventMessage($langs->trans('CHISuccessAddVoyageLines', $numberLineCreate));
	...
	if ($action == "confirm-add-to-project") {
		...
		if ($res > 0) {
			if ($project->status != Project::STATUS_CLOSED) {
				foreach ($toselect as $voyageId) {
					$voyage = new Voyage($db);
					$resVoyage = $voyage->fetch($voyageId);

					if ($resVoyage > 0) {
						$taskFromVoyage = new Task($db);
						$labelTaskFromVoyage = new Product($db);
						$resLabel = $labelTaskFromVoyage->fetch($voyage->fk_product);
						...
						$taskFromVoyage->fk_project = $target_fk_projet;
						$taskFromVoyage->fk_task_parent = 0;

						if(!empty($voyage->commercial_text)) {
							...
							$taskFromVoyage->description .= $voyage->commercial_text;
						}

						if(!empty($voyage->detailed_feature_specification)){
							if(!empty($taskFromVoyage->description)){
								$taskFromVoyage->description.= "\n";
							}
							$taskFromVoyage->description.= '<h4>'.$langs->trans('DetailedFeatureSpecification').'</h4>'."\n";
							$taskFromVoyage->description.= $voyage->detailed_feature_specification;
						}

						if(!empty($voyage->tech_detail)){
							if(!empty($taskFromvoyage->description)){
								$taskFromvoyage->description.= "\n";
							}
							$taskFromvoyage->description.= '<h4>'.$langs->trans('CHITechDetail').'</h4>'."\n";
							$taskFromvoyage->description.= $voyage->tech_detail;
						}


						//Ajout de l'extrafield voyage sur tâche en cours de création
						$taskFromvoyage->array_options['options_fk_voyage'] = $voyage->id;

						$taskFromvoyage->planned_workload = ($conf->global->VOYAGE_DEFAULT_MULTIPLICATOR_FOR_TASK * 3600) * $voyage->qty;
						$res = $taskFromvoyage->create($user);

						$voyage->add_object_linked('project_task', $taskFromvoyage->id);
	...
	$objectclass = 'Voyage';
	$objectlabel = 'Voyage';
	$uploaddir = $conf->voyage->dir_output;
...
if ($num == 1 && !empty($conf->global->MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE) && $search_all && !$page) {
	...
	$id = $obj->rowid;
	header("Location: " . dol_buildpath('/voyage/voyage_card.php', 1) . '?id=' . $id);
	exit;
}
...
if ($socid > 0 && !empty($user->rights->societe->lire)) {
	...
	if ($res > 0) {
		...
		print dol_get_fiche_head($head, 'tabvoyage', $langs->trans("ThirdParty"), -1, 'company');
...
$linktoadd = dol_buildpath('/voyage/voyage_card.php', 1) . '?action=create' . $addparams;
$newcardbutton = dolGetButtonTitle($langs->trans('New'), '', 'fa fa-plus-circle', $linktoadd, '', $permissiontoadd);

print_barre_liste($title, $page, $_SERVER["PHP_SELF"], $param, $sortfield, $sortorder, $massactionbutton, $num, $nbtotalofrecords, 'object_voyage_titre@voyage', 0, $newcardbutton, '', $limit, 0, 0, 1);
...
//TODO Mass action from voyage list to propal lines
if ($massaction == 'preaddpropal') {
	...
	$objectStaticForMassAction = new Voyage($db);
	print '<input type="hidden" name="token" value="' . newToken() . '" />';
...
if (in_array('builddoc', $arrayofmassactions) && ($nbtotalofrecords === '' || $nbtotalofrecords)) {
	...
	print $formfile->showdocuments('massfilesarea_voyage', '', $filedir, $urlsource, 0, $delallowed, '', 1, 1, 0, 48, 1, $param, $title, '', '', '', null, $hidegeneratedfilelistifempty);
}
