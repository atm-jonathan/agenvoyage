...
/**
 *    \file       chiffrage_list.php
 *        \ingroup    chiffrage
 *        \brief      List page for chiffrage
 */
...
// load chiffrage libraries
require_once __DIR__ . '/class/chiffrage.class.php';
...
$hookmanager->initHooks(array('chiffragelist')); // Note that conf->hooks_modules contains array
...
$permissiontodelete = $user->rights->chiffrage->chiffrage->delete;

// Security check
if (empty($conf->chiffrage->enabled)) {
...
if (empty($reshook)) {
	...
	// Mass actions
	$objectclass = 'Chiffrage';
	$objectlabel = 'Chiffrage';
	$uploaddir = $conf->chiffrage->dir_output;
...
//$help_url="EN:Module_Chiffrage|FR:Module_Chiffrage_FR|ES:Módulo_Chiffrage";
$help_url = '';
$title = $langs->trans('ListOf', $langs->transnoentitiesnoconv("Chiffrages"));
$morejs = array();
$morecss = array();
...
if ($num == 1 && !empty($conf->global->MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE) && $search_all && !$page) {
	...
	$id = $obj->rowid;
	header("Location: " . dol_buildpath('/chiffrage/chiffrage_card.php', 1) . '?id=' . $id);
	exit;
}
...
if (in_array('builddoc', $arrayofmassactions) && ($nbtotalofrecords === '' || $nbtotalofrecords)) {
	...
	print $formfile->showdocuments('massfilesarea_chiffrage', '', $filedir, $urlsource, 0, $delallowed, '', 1, 1, 0, 48, 1, $param, $title, '', '', '', null, $hidegeneratedfilelistifempty);
}
